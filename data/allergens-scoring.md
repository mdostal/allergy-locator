...
Plain-language spec for `data/allergens.json`, generated by
`scripts/source_allergens.py` (re-runnable). Covers every allergen beyond the
original 15-species author panel (`data/species-ranges.json`) and grass's
ground-truth-fit formula (`data/allergy-scoring.md`) — per the explicit
instruction to pull and include every allergen there's sourceable data for, not
a curated subset.
